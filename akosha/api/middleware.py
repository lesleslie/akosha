...
async def require_role(required_role: str) -> Callable[[dict[str, Any]], dict[str, Any]]:
    ...
    def dependency(claims: dict[str, Any] = Depends(verify_token)) -> dict[str, Any]:  # noqa: B008
        ...
        if role not in roles:
            logger.warning(
                f"User {claims.get('sub')} attempted to access {role} resource with roles: {roles}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
def audit_log(_action: str, _resource_type: str) -> Callable[[dict[str, Any]], AuditLogger]:  # type: ignore[reportUnusedParameter]
    ...
    def dependency(_claims: dict[str, Any] = Depends(verify_token)) -> AuditLogger:  # type: ignore[reportUnusedParameter] # noqa: B008
...
class RBACMiddleware:
    ...
    def has_permission(self, role: str, permission: str) -> bool:
        ...
        """
        role_perms: list[str] = list(self.role_permissions.get(role, []))
...
async def require_permission(permission: str) -> Callable[[dict[str, Any]], dict[str, Any]]:
    ...
    def dependency(claims: dict[str, Any] = Depends(verify_token)) -> dict[str, Any]:  # noqa: B008
        ...
        # Note: Synchronously check permission since we can't await in a sync dep
        roles = claims.get("roles", [Role.VIEWER])
        has_permission = False
        for role in roles:
