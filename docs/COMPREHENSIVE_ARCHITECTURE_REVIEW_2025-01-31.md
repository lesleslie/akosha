...
2. Sequential upload processing (3× slower than target)
3. Sharding layer missing (cannot distribute data)
4. Tier aging service missing (storage will bloat)
...
2. Tune HNSW parameters: `m=32, ef_construction=400, ef_search=200`
3. **Earlier Milvus migration**: Move to Milvus at 10M embeddings (not 100M)
...
2. Add accuracy monitoring (recall@k metrics)
3. Re-ranking strategy: Use INT8 for retrieval, re-rank top-100 with FLOAT
...
2. **Three-Tier Storage (8.5/10)** - Hot/Warm/Cold with 80% cost reduction is sound
3. **Circuit Breaker Implementation (9/10)** - Three-state system with 98.78% test coverage
4. **OpenTelemetry Observability (9/10)** - Comprehensive tracing and metrics
5. **ADR-001 Documentation (9/10)** - Excellent decision documentation with rationale
...
2. Add concurrent upload processing (1 day)
3. Implement sharding layer (2-3 days)
...
13. Implement layered caching (L1 + L2)
14. Fix async/sync pattern (run DuckDB in executor)
15. Implement MinHash fuzzy deduplication
16. Add backlog alerting to Prometheus
17. Complete Kubernetes manifests (PVC, ConfigMap, Secret)

### Long-Term (Phase 3+) - 8-12 weeks

18. Migrate to Milvus at 10M embeddings (earlier than planned)
19. Implement product quantization for better compression
20. Add shard rebalancing for hotspots
21. Document migration strategy (DuckDB → Milvus)
22. Implement backup and disaster recovery
23. Add multi-region replication
...
2. **Deduplication Effectiveness**: Target >90% detection, <1% false positive
3. **Tier Consistency**: Target >98% result overlap across tiers
...
2. Data transformation: FLOAT[384] → Milvus collection schema
3. Index rebuilding: HNSW index construction (100M embeddings = 24-48 hours)
4. Query rewriting: All vector queries must use Milvus API
5. Validation: Verify search results match between DuckDB and Milvus
...
2. Add sharding layer with consistent hashing
3. Implement tier aging service
4. Add distributed query engine
5. Complete Oneiric integration
6. Fix async/sync pattern
