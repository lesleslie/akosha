# Akosha Universal Memory Aggregation System - Storage Architecture
...
Akosha is a universal memory aggregation system designed to ingest and store massive amounts of data from 100-10,000+ Session-Buddy systems. This document defines the comprehensive Oneiric-based storage architecture that provides tiered storage, cross-cloud redundancy, and seamless scalability.
...
Akosha leverages Oneiric's universal resolution layer with four key patterns:
...
storage:
  ...
  s3-cold:
    provider: "s3"
    bucket_name: "${AKOSHA_COLD_BUCKET:akosha-cold-${env}}"
    region: "${AWS_REGION:us-east-1}"
    storage_class: "GLACIER"
    tier: "cold"
...
storage:
  ...
  azure-secondary:
    provider: "azure"
    container_name: "akosha-secondary"
    storage_class: "HOT"
...
storage:
  ...
  s3-cloud:
    provider: "s3"
    bucket_name: "akosha-cloud-backup"
    region: "us-east-1"
    tier: "warm"
    # Cost optimization
```
...
class AkoshaStorageSettings:
    ...
    def from_settings() -> Settings:
        ...
        Searches in order:
        1. settings/akosha-storage.yml (project-specific)
        2. ~/.config/oneiric/settings/storage.yml (user-specific)
        3. Environment variables (highest priority)
        """
        config_path = Path("settings/akosha-storage.yml")
    ...
    def get_storage_adapter(tier: str = "hot") -> Any:
        ...
        # Register Akosha storage adapters
        AkoshaStorageSettings._register_adapters(resolver)
    ...
    async def search_similar(
        ...
        for path in scan_paths:
            await db.execute(f"""
                ...
                SELECT * FROM read_parquet('s3://akosha-vectors/{path}')
            """)
...
### 5.1 Akosha Storage Adapter Registration
...
def register_akosha_storage_adapters(resolver: Resolver) -> None:
    ...
    This enables Akosha to leverage Oneiric's 4-tier resolution system,
    ...
    register_adapter_metadata(
        ...
        adapters=[
            ...
            AdapterMetadata(
                ...
                stack_level=50,
                factory=lambda: DuckDBAdapter(
                    database_path="/data/akosha/warm.duckdb",
                    read_only=False,
                ),
...
class MultiCloudCoordinator:
    ...
    def __init__(self):
        self.primary_region = os.getenv("AKOSHA_PRIMARY_REGION", "us-east-1")
...
```yaml
# settings/akosha-storage.yml (production)
storage:
  default_backend: "s3-hot"
...
```yaml
# settings/akosha-storage-dev.yml (development)
storage:
  default_backend: "file"

  # Development: Local file storage
  file-dev:
    local_path: "${PWD}/data/akosha/dev"
    tier: "hot"
...
class AkoshaStorageConfig(BaseModel):
    ...
    @classmethod
    def from_yaml(cls, config_path: str | Path) -> "AkoshaStorageConfig":
